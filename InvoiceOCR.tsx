...
import { InboxItem, InvoiceData, InboxItemStatus, Toast } from '../types';
...
const InboxItemCard: React.FC<{
    ...
    const handleApprove = async () => {
        if (!localData) return;
        setIsApproving(true);
        // Create a new item object with the latest localData to pass up
                    ...
                    <div className="flex justify-between items-center mb-3">
                        ...
                        <div className="flex items-center gap-2">
                            {item.status === 'pending_review' && (
                                <button onClick={handleSave} disabled={isSaving} className="p-2 text-slate-500 hover:text-blue-600 disabled:opacity-50">
                                    {isSaving ? <Loader className="w-5 h-5 animate-spin" /> : <Save className="w-5 h-5" />}
                                </button>
                            )}
                            <button onClick={handleDelete} disabled={isDeleting} className="p-2 text-slate-500 hover:text-red-600 disabled:opacity-50">
...
const InvoiceOCR: React.FC<InvoiceOCRProps> = ({ onSaveExpenses, addToast }) => {
        ...
        <div className="space-y-6">
            ...
            {isLoading ? (
                ...
                items.length > 0 ? (
                    <div className="space-y-6">
                        {items.map(item => (
                            <InboxItemCard key={item.id} item={item} onUpdate={handleUpdateItem} onDelete={handleDeleteItem} onApprove={handleApproveItem} />
                        ))}
                    </div>
                ) : (
