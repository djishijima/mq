...
const DailyReportForm: React.FC<DailyReportFormProps> = ({ isDemoMode }) => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            await submitApplication({
                applicationCodeId: 'DLY',
                formData,
                approvalRoute
            });
            setSuccess('日報を提出しました。');
        } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-sm space-y-6">
            <h2 className="text-2xl font-bold text-slate-800 dark:text-white">日報作成</h2>
             ...
             <div className="text-center p-8 border-2 border-dashed border-slate-300 dark:border-slate-600 rounded-lg">
                <p className="text-slate-500 dark:text-slate-400">日報のフォーム項目はここに表示されます。</p>
