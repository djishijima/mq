...
const ApprovalForm: React.FC<ApprovalFormProps> = ({ onSuccess, applicationCodeId, currentUser }) => {
        ...
        <>
            <form onSubmit={handleSubmit} className="bg-white dark:bg-slate-800 p-8 rounded-2xl shadow-sm space-y-6">
                ...
                <div>
                    ...
                    <input type="text" id="title" name="title" value={formData.title} onChange={handleChange} className={inputClass} required disabled={isSubmitting} placeholder="例: 新規取引先との契約締結について" />
                </div>

                <div>
                    <label htmlFor="details" className={labelClass}>目的・概要 *</label>
                    <textarea id="details" name="details" rows={6} value={formData.details} onChange={handleChange} className={inputClass} required disabled={isSubmitting} placeholder="申請する決裁の目的、背景、具体的な内容などを記述してください。" />
                </div>
