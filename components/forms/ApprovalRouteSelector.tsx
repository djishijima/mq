...
const ApprovalRouteSelector: React.FC<ApprovalRouteSelectorProps> = ({ onChange, isSubmitting }) => {
        ...
        const fetchRoutes = async () => {
            try {
                ...
                if (fetchedRoutes.length > 0) {
                    // Pre-select the first route
                    const firstRouteId = fetchedRoutes[0].id;
                    setSelectedRoute(firstRouteId);
                    onChange(firstRouteId);
                }
            } catch (error) {
