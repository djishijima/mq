                        ...
                        return (
                            <div key={item.id} className="p-3 border border-slate-200 dark:border-slate-700 rounded-lg mb-3 space-y-2">
                                <div className="grid grid-cols-1 md:grid-cols-12 gap-2 items-start">
                                    ...
                                    <div className={`md:col-span-3 ${validationErrors.has(`${item.id}-paymentRecipientId`) ? 'p-1 bg-red-100 rounded-lg shake' : ''}`}>
                                        <PaymentRecipientSelect value={item.paymentRecipientId} onChange={id => handleDetailChange(item.id, 'paymentRecipientId', id)} required />
                                    </div>
                                    <input type="text" value={item.description} onChange={e => handleDetailChange(item.id, 'description', e.target.value)} placeholder="内容" className={`${inputClass} md:col-span-4 ${validationErrors.has(`${item.id}-description`) ? 'border-red-500 shake' : ''}`} disabled={isDisabled} aria-label="内容" />
                                    <input type="number" value={item.amount} onChange={e => handleDetailChange(item.id, 'amount', Number(e.target.value))} placeholder="金額" className={`${inputClass} md:col-span-2 text-right ${validationErrors.has(`${item.id}-amount`) ? 'border-red-500 shake' : ''}`} disabled={isDisabled} aria-label="金額" />
                                    <button type="button" onClick={() => handleRemoveRow(item.id)} className="p-2 text-slate-400 hover:text-red-500 md:col-span-1" disabled={isDisabled} aria-label="明細を削除"><Trash2 className="w-5 h-5"/></button>
                                </div>
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                                    <div className={`${validationErrors.has(`${item.id}-accountItemId`) ? 'p-1 bg-red-100 rounded-lg shake' : ''}`}>
                                        <AccountItemSelect value={item.accountItemId} onChange={id => handleDetailChange(item.id, 'accountItemId', id)} required />
                                    </div>
                                    <div className={`${validationErrors.has(`${item.id}-allocationDivisionId`) ? 'p-1 bg-red-100 rounded-lg shake' : ''}`}>
