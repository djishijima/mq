...
import { Loader, PlusCircle, X, Save, Trash2, Pencil, Users } from '../Icons';
...
const UserManagementPage: React.FC<UserManagementPageProps> = ({ addToast }) => {
        ...
        <div className="bg-white dark:bg-slate-800 rounded-2xl shadow-sm">
            ...
            {isLoading ? (
                ...
                <EmptyState
                    icon={Users}
                    title="ユーザーがいません"
                    message="最初のユーザーを登録してください。"
                    action={{label: "新規ユーザー追加", onClick: () => handleOpenModal(), icon: PlusCircle}}
                />
            ) : (
                            ...
                            <tr key={user.id} className="border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex justify-center items-center gap-2">
                                        ...
                                        <button onClick={() => handleDeleteUser(user.id)} className="p-2 text-slate-500 hover:text-red-600"><Trash2 className="w-5 h-5" /></button>
                                    </div>
                                </td>
                            </tr>
                        ))}
