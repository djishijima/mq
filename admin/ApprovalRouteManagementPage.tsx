...
import { ApprovalRoute, User, Toast } from '../../types';
...
const ApprovalRouteManagementPage: React.FC<ApprovalRouteManagementPageProps> = ({ addToast }) => {
    ...
    const handleDeleteRoute = async (routeId: string) => {
        if (window.confirm('この承認ルートを本当に削除しますか？このルートを使用している申請に影響が出る可能性があります。')) {
            try {
                            ...
                            <tr key={route.id} className="border-b dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-700/50">
                                ...
                                <td className="px-6 py-4">
                                    <div className="flex justify-center items-center gap-2">
                                        ...
                                        <button onClick={() => handleDeleteRoute(route.id)} className="p-2 text-slate-500 hover:text-red-600"><Trash2 className="w-5 h-5" /></button>
                                    </div>
                                </td>
                            </tr>
                        ))}
